...
float cnoise(vec3 P){
  ...
  vec3 g001 = vec3(gx1.x,gy1.x,gz0.x);
  vec3 g101 = vec3(gx1.y,gy1.y,gz0.z);
  vec3 g011 = vec3(gx1.z,gy1.z,gz0.z);
...
export interface BeamsBackgroundProps extends Partial<BeamsProps> {
  className?: string;
}

/**
 * BeamsBackground renders the animated 3D beams inside an absolutely
 * positioned container. Falls back to a static gradient when the user
...
export function BeamsBackground({
  ...
  if (reduceMotion) {
    return (
      <div
        className={`bg-[radial-gradient(ellipse_at_50%_0%,rgba(255,255,255,0.10),transparent_60%)] ${className}`}
      />
    );
  }
