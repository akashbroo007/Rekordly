...
async function main() {
  ...
  if (!username) {
    console.log('searching homepage for live models...');
    const search = await plugin.capabilities.creatorSearch.search({ query: '', limit: 5 });
    console.log('found', search.creators.length, 'models');
    if (search.creators.length === 0) {
      console.error('FAIL: no models discovered from homepage');
      process.exit(1);
    }
    username = search.creators[0].username;
  }
