...
// ponytail: jerkmates.com is a Streamate white-label — same Naiad Systems
// backend (verified live: the shared manifest edge answers for Jerkmate
// nicknames). The white-label only owns the storefront (site URL + room page
// paths); stream state and HLS playlists come from the network edge, so
// changing the site URL never changes where recordings are pulled from.
const DEFAULT_BASE = 'https://jerkmates.com';
/**
 * Naiad Systems' stream manifest edge, shared by every Streamate white-label.
 * `format=mp4-hls` returns HLS encoding tables; HTTP 200 with a `formats`
 * object means the broadcast is up.
 */
...
const SETTINGS_SCHEMA: Record<string, SettingDef> = {
  baseUrl: {
    type: 'text',
    label: 'Site URL',
    description: 'Base URL of the Jerkmate white-label (room pages live at /camgirl/<name>/).',
  ...
  manifestBaseUrl: {
    ...
    description:
      'Naiad Systems manifest edge serving the live stream playlists (shared by the Streamate network).',
...
function request(
  ...
  ctx?.logger.debug(`jerkmate: ${options.method ?? 'GET'} ${url}`);
    ...
    const req = mod.request(
      ...
      (res) => {
        ...
        if (statusCode >= 300 && statusCode < 400 && res.headers.location) {
          ...
          ctx?.logger.debug(`jerkmate: ${statusCode} -> redirect to ${next}`);
          res.resume();
        ...
        res.on('end', () => {
          ...
          ctx?.logger.debug(`jerkmate: ${statusCode} ${body.length} bytes from ${url}`);
...
async function withSmartRoute<T>(
  ...
  try {
    ...
    ctx?.logger.debug(
      `jerkmate: direct request failed (${err instanceof Error ? err.message : err}) — retrying through secure proxy`,
    );
  }
    ...
    variants.push({
      ...
      bandwidth:
        typeof enc.videoKbps === 'number' && enc.videoKbps > 0
          ...
          : (fromUrl?.bandwidth ?? 0),
      resolution:
        width !== undefined && height !== undefined ? `${width}x${height}` : fromUrl?.resolution,
    });
  }
  return variants;
}
...
async function resolveStreamSmart(
  ...
  if (!manifestResult.ok) {
    ...
    throw new Error(manifestResult.error ?? 'manifest unavailable');
  }
  if (!hasHlsEncodings(manifestResult.info)) {
  ...
  const variants = variantsFromManifest(manifestResult.info);
  if (variants.length === 0) {
    return null;
  }
  ...
  const resolved = await resolveBestVariantUrl(
    best.url,
    manifestResult.usedProxy ? await proxyFor(identifier) : null,
  );
  ...
  return { url: bestUrl, variants: finalVariants, usedProxy: manifestResult.usedProxy };
}

// ---- creator helpers -------------------------------------------------------

// ponytail: the room page path is /camgirl/<name>/ on the white-label
// storefront. Only the storefront URL pattern differs from streamate.com's
// /cam/<name>/ — stream state always comes from the shared manifest edge.
function roomUrl(username: string): string {
  return `${getBase()}/camgirl/${encodeURIComponent(username)}/`;
}

function creatorFromUsername(username: string, live: boolean): CreatorInfo {
  return {
    id: `jerkmate:${username}`,
    username,
    displayName: username,
    profileUrl: roomUrl(username),
    isLive: live,
    liveUrl: live ? roomUrl(username) : undefined,
  };
}
...
const plugin: Plugin = {
  ...
  capabilities: {
    auth: {
      ...
      async login(credentials?: Record<string, string>): Promise<void> {
        ...
        ctx?.logger.info('jerkmate: cookies loaded');
      },
      async logout(): Promise<void> {
        cookieJar = {};
        ctx?.logger.info('jerkmate: cookies cleared');
      },
    },

    creatorSearch: {
      async search(query: CreatorSearchQuery): Promise<CreatorSearchResult> {
        const q = query.query.trim().replace(/^jerkmate:/, '');
        if (!q) return { creators: [], total: 0 };
        // ponytail: the white-label exposes no public search API — resolve
        // the nickname directly (manifest edge + room page). Same policy as
        // the Streamate/BongaCams plugins: search = username lookup.
      ...
      async getCreator(identifier): Promise<CreatorInfo | null> {
        const username = identifier.replace(/^jerkmate:/, '');
        ...
        try {
          ...
          // server-side performer data, so the nickname doubles as the
          // display name — same as the Streamate plugin's fallback.
          const manifestResult = await fetchLiveManifestSmart(identifier, username);
    ...
    liveDetection: {
      async getLiveStatus(identifier): Promise<LiveStatus> {
        const username = identifier.replace(/^jerkmate:/, '');
        ctx?.logger.debug(`jerkmate: checking live status for ${username}`);
        ...
        try {
          ...
          if (resolved === null) {
            ctx?.logger.debug(`jerkmate: ${username} manifest gone (404/no encodings) — offline`);
            return { isLive: false };
          }
          ctx?.logger.debug(
            `jerkmate: ${username} live (${resolved.variants.length + 1} encodings)`,
          );
          return {
            isLive: true,
            title: `${username} is live`,
            // ponytail: the manifest payload carries no live viewer count or
            // room title — omit rather than show a wrong number (the UI only
          ...
          // retries instead.
          ctx?.logger.warn(
            `jerkmate: ${username} check degraded (site unreachable) — host will retry: ${msg}`,
          );
    ...
    streamExtraction: {
      async extractStream(identifier): Promise<StreamObject> {
        const username = identifier.replace(/^jerkmate:/, '');
        ctx?.logger.debug(`jerkmate: extracting stream for ${username}`);
        const resolved = await resolveStreamSmart(identifier, username);
        if (resolved === null) {
        ...
        if (proxyUrl !== null) {
          ctx?.logger.debug('jerkmate: stream will be recorded through the secure proxy');
        }

        return {
          creatorId: `jerkmate:${username}`,
          creatorName: username,
          platformId: 'jerkmate',
          title: `${username} live on Jerkmate`,
          streamUrl: resolved.url,
          headers: siteHeaders({ Referer: `${getBase()}/` }),
          cookies,
      ...
      async check(): Promise<HealthStatus> {
        ...
        try {
          ...
          return {
            ...
            message: res.status === 200 ? 'Jerkmate reachable' : `HTTP ${res.status}`,
    ...
    metadata: {
      async getCreatorMetadata(identifier): Promise<CreatorMetadata> {
        const username = identifier.replace(/^jerkmate:/, '');
        // ponytail: room pages are client-rendered (no SSR performer data),
        // so metadata is limited to what the platform exposes without an
        // authenticated session — same as the Streamate plugin.
        return {
          displayName: username,
          website: roomUrl(username),
        };
      },
    },
  },
  ...
  async initialize(context: PluginContext): Promise<void> {
    ...
    ctx = context;
    context.logger.info('jerkmate: initializing');
    ...
    if (typeof savedCookies === 'string' && savedCookies.length > 0) {
      ...
      context.logger.info('jerkmate: loaded saved cookies');
    }
    status.state = 'ready';
  },
